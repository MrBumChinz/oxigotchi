...
# Skip PWND display entirely in AO mode — AO indicator shows captures
old_pwnd = '''        tot = utils.total_unique_handshakes(self._config['bettercap']['handshakes'])
...
new_pwnd = '''        # In AO mode, don't update PWND display — AO indicator handles capture count.
        if self._ao_mode:
            return
        ...
        self._view.set('shakes', txt)'''
if old_pwnd in code and 'AO indicator handles capture count' not in code:
